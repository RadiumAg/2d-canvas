...
export class vec2 {
  ...
  public reset(x = 0, y: number): vec2 {
    this.values[0] = x;
    this.values[1] = y;
    return this;
  }
  ...
  public static getOrientation(from: vec2, to: vec2, isRadian = false): number {
  ...
  public static getAngle(a: vec2, b: vec2, isRadian = false): number {
  ...
  public static cosAngle(a: vec2, b: vec2, norm = false): number {
    if (norm === true) {
      a.normalize();
      b.normalize();
    }
  ...
  public static sinAngle(a: vec2, b: vec2, norm = false): number {
    if (norm === true) {
      a.normalize();
      b.normalize();
    }
...
export class mat2d {
  ...
  public constructor(a = 1, b = 0, c = 0, d = 1, x = 0, y = 0) {
  ...
  public static create(a = 1, b = 0, c = 0, d = 1, x = 0, y = 0): mat2d {
  ...
  public getAngle(isRadian = false): number {
    const angle: number = Math.atan2(this.values[1], this.values[0]);
    if (isRadian) {
      return angle;
    }
  ...
  public static multiply(
    ...
    const b0: number = right.values[0];
    const b1: number = right.values[1];
    const b2: number = right.values[2];
    const b3: number = right.values[3];
    const b4: number = right.values[4];
    const b5: number = right.values[5];
...
export class MatrixStack {
  ...
  public translate(x = 0, y = 0): void {
    const mat: mat2d = mat2d.makeTranslation(x, y);
    this.multMatrix(mat);
  }

  public rotate(degree = 0, isRadian = false): void {
    if (isRadian === false) {
      degree = Math2D.toRadian(degree);
    }
    const mat: mat2d = mat2d.makeRotation(degree);
    this.multMatrix(mat);
  }

  public rotateFrom(v1: vec2, v2: vec2, norm = false): void {
    const mat: mat2d = mat2d.makeRotationFromVectors(v1, v2, norm);
    this.multMatrix(mat);
  }

  public scale(x = 1.0, y = 1.0): void {
...
export class Math2D {
  ...
  public static projectPointOnLineSegment(
    ...
    const t: number = vec2.dotProduct(v0, v1);
    if (t < 0) {
...
export class Size {
  ...
  public static create(w = 1, h = 1): Size {
    return new Size(w, h);
  }
}
...
export class Rectangle {
  ...
  public static create(x = 0, y = 0, w = 1, h = 1): Rectangle {
    const origin: vec2 = new vec2(x, y);
    const size: Size = new Size(w, h);
...
export class Inset {
  ...
  public constructor(l = 0, t = 0, r = 0, b = 0) {
...
export class Transform2D {
  ...
  public constructor(x = 0, y = 0, rotation = 0, scaleX = 1, scaleY = 1) {
...
export class BezierEnumerator implements IBezierEnumerator {
  ...
  public get current(): vec2 {
    if (this._controlPoint1 !== null) {
      return Math2D.getCubicBezierVector(
        this._startAnchorPoint,
        this._controlPoint0,
        this._controlPoint1,
        this._endAnchorPoint,
        this._currentIdx * this._i,
      );
    } else {
      return Math2D.getQuadraticBezierVector(
        this._startAnchorPoint,
        this._controlPoint0,
        this._endAnchorPoint,
        this._currentIdx * this._i,
...
export class QuadraticBezierEnumerator implements IBezierEnumerator {
  ...
  public constructor(start: vec2, end: vec2, control0: vec2, steps = 30) {
    this._startAnchorPoint = start;
    this._endAnchorPoint = end;
    this._controlPoint0 = control0;
    this._steps = steps;
  ...
  public get current(): vec2 {
    const t: number = this._currentIdx * this._i;
    const ret: vec2 = vec2.create(t * t, t);
    Math2D.transform(mat2d.quadBezierBasicMatrix, ret, ret);
    ret.x =
      this._startAnchorPoint.x * ret.x +
      this._controlPoint0.x * ret.y +
      this._endAnchorPoint.x;
    ret.y =
      this._startAnchorPoint.y * ret.x +
      this._controlPoint0.y * ret.y +
